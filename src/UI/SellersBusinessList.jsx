import Dropdown from "./SortBy"

function SellersBusinessList() {
    const sellers = [
        {
            business_name: "Luxe Lashes By Sarah",
            description: "Premium eyelash extensions and eyebrow mapping servies right in your village.",
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <p className="text-neutral-100 text-sm">{sellers.length} results</p>
                <div className="flex items-center gap-1">
                    {/* <span className="">SortBy:</span> */}
                    <Dropdown />
                </div>
            </div>
            <div className="grid grid-cols-[repeat(auto-fit,minmax(130px,1fr))] lg:grid-cols-4 gap-2 md:gap-4">
                {sellers.map((seller, index) => (
                    <div key={index} className="rounded-xl lg:rounded-2xl overflow-hidden bg-white ">
                        <div className="relative h-25 lg:h-[219.98px]">
                            ...
                            <span className="absolute top-2 right-2 py-0 lg:py-1 px-3 bg-white/90 rounded-full text-primary font-medium text-xs lg:text-sm capitalize lg:top-4 lg:right-4">{seller.category_name}</span>
                        </div>
                        <div className="p-2 space-y-1 lg:p-6 lg:space-y-2">
                            ...
                            <p className="lg:text-base text-neutral-100 line-clamp-2 lg:line-clamp-3">{seller.description}</p>
...
export default SellersBusinessList
